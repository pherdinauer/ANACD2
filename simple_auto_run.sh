...
# ANAC JSON Downloader - Simple Auto Run
# Script semplificato per pull e avvio automatico
...
echo -e "${BLUE}==========================================${NC}"
echo -e "${BLUE}  ANAC JSON DOWNLOADER - AUTO RUN${NC}"
echo -e "${BLUE}==========================================${NC}"
...
echo -e "${YELLOW}[INFO]${NC} Avvio script auto-run..."

# Funzione per pulire e uscire
cleanup() {
    echo -e "${YELLOW}[INFO]${NC} Interruzione rilevata. Pulizia..."
    if [ -n "$APP_PID" ]; then
...
# Avvia applicazione
echo -e "${GREEN}[SUCCESS]${NC} Tutto pronto! Avvio applicazione..."
echo -e "${BLUE}==========================================${NC}"
...
echo -e "${YELLOW}[INFO]${NC} Applicazione avviata con PID: $APP_PID"
echo -e "${YELLOW}[INFO]${NC} Per fermare: Ctrl+C"
